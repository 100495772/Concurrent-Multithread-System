...
pthread_mutex_t mremaining_producers; /*mutex to access the remaining producers global variable*/
int remaining_producers; /*Global variable representing the number of producer threads that have yet to finish*/

int purchase_cost[] = {2, 5, 15, 25, 100}; // Cost of each purchase operation
int selling_price[] = {3, 10, 20, 40, 125}; // Price profit obtained from each sale operation

struct Transaction {
  int product_id;
  const char
      operation[MAX_OPERATION_LENGTH]; // Operation type: PURCHASE or SALE
  int units;
};
...
// Structure used to receive the results from the consumer threads
...
void *consumer(void *args) {
  ...
  // Allocate memory for the consumer result structure an set the attributes to 0
  struct consumer_result *c_result =
      (struct consumer_result *)malloc(sizeof(struct consumer_result));
  c_result->profit = 0;
  for (int i = 0; i < 5; i++) {
    c_result->partial_stock[i] = 0;
  }

  // Infinite loop to consume elements from the queue
  for (;;) {
    // Lock the mutex to access the queue
    pthread_mutex_lock(&mutex);
    // Wait until the queue is not empty
    while (queue_empty(q)) {
      // If the queue is empty and all producers have finished, unlock the mutex and exit returning the result
    ...
    // Get the next element from the queue and signal tha the queue is not empty
    ...
    if (operation == 0) { // Purchase operation
      ...
      // Change the profit and partial stock from the result struct
    ...
    // Free the memory for the dequeued element (the memory allocation is made
    // in the queue_get())
    free(element);
  }
}
...
void *producer(void *args) {
  ...
  for (int i = start_index; i < end_index; i++) {
    ...
    struct element elem = {operations[i].product_id, local_operation,
                           operations[i].units};
...
int main(int argc, const char *argv[]) {
  ...
  if (argc != 5) {
    fprintf(
        stderr,
        "Usage: %s <file_name> <num_producers> <num_consumers> <buffer_size>\n",
        argv[0]);
    return 1;
  }
  ...
  //If the producers or consumers are a negative quantity, give an error
  if (atoi(argv[2]) < 0 || atoi(argv[3]) < 0){
    fprintf(stderr, "Producers and consumers must be a positive integer or 0\n");
    return 1;
  }
  ...
  //When buffer size is 0, give an error
  if (atoi(argv[4]) < 1){
    fprintf(stderr, "The buff size must be a positive integer");
    return 1;
  }
  ...
  struct Transaction *operations =
      malloc(num_operations * sizeof(struct Transaction));
  if (operations == NULL) {
  ...
  for (int i = 0; i < num_operations; i++) {
    if (fscanf(file, "%d %s %d", &operations[i].product_id,
               (char *)&operations[i].operation, &operations[i].units) != 3) {
  ...
  for (int i = 0; i < num_producers; i++) {
    struct producer_args *thread_args =
        (struct producer_args *)malloc(sizeof(struct producer_args));
    if (thread_args == NULL) {
    ...
    first remaining_operations threads receive one additional operation compared
    to the rest, to ensure fair distribution*/
    ...
    if (pthread_create(&producers[i], NULL, producer, (void *)thread_args) !=
        0) {
  ...
  for (int i = 0; i < num_consumers; i++) {
    // Receive the result of the consumer thread (consumer profit and partial
    // stock)
    ...
    struct consumer_result *result_pointer =
        (struct consumer_result *)consumer_result;
    // Modify the total profit and total stock based on the consumer profit and
    // partial stock
    profits += result_pointer->profit;
    for (int j = 0; j < 5; j++) {
      product_stock[j] += result_pointer->partial_stock[j];
    }
    free(result_pointer); // Free the memory allocated for the consumer result
                          // struct
  }
